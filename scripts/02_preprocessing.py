        ...
        for session in data['sessions']:
            ...
            if 'start' in normalized_session:
                try:
                    ...
                    normalized_session['start'] = start_time.strftime('%Y-%m-%dT%H:%M:%S')
                except:
                    logger.warning(f"無法解析開始時間: {normalized_session.get('start')}")
            
            if 'end' in normalized_session:
                try:
                    end_time = pd.to_datetime(normalized_session['end'])
                    normalized_session['end'] = end_time.strftime('%Y-%m-%dT%H:%M:%S')
                except:
                    logger.warning(f"無法解析結束時間: {normalized_session.get('end')}")
            
            # 處理講者欄位 - 確保為列表格式
            if 'speakers' in normalized_session:
                speakers = normalized_session['speakers']
...
def clean_and_standardize_data(sessions_df, speakers_df, tracks_df):
    ...
    if not speakers_df.empty:
        ...
        if 'year' in speakers_df.columns:
            speakers_df['year'] = pd.to_numeric(speakers_df['year'], errors='coerce').astype('Int64')
    ...
    if not tracks_df.empty:
        ...
        if 'year' in tracks_df.columns:
            tracks_df['year'] = pd.to_numeric(tracks_df['year'], errors='coerce').astype('Int64')
    ...
    if 'name' in unique_speakers.columns:
        ...
        for name, group in name_groups:
            if len(group) > 1 and len(set(group['id'])) > 1:
                # 名稱相同但ID不同，可能是同一人
                logger.info(f"發現可能相同的講者: {name}, IDs: {group['id'].tolist()}")
                
                # 使用最常見的ID作為唯一ID
                most_common_id = group['id'].value_counts().index[0]
                
                # 更新唯一ID
                unique_speakers.loc[unique_speakers['name'] == name, 'unique_id'] = most_common_id
...
def merge_yearly_data(yearly_data):
    ...
    # 合併DataFrame
    merged_sessions_df = pd.concat(all_sessions, ignore_index=True) if all_sessions else pd.DataFrame()
    merged_speakers_df = pd.concat(all_speakers, ignore_index=True) if all_speakers else pd.DataFrame()
    merged_tracks_df = pd.concat(all_tracks, ignore_index=True) if all_tracks else pd.DataFrame()
    ...
    if not merged_speakers_df.empty:
        ...
        # 去除重複講者 (保留最新年份的資料)
        if 'unique_id' in merged_speakers_df.columns and 'year' in merged_speakers_df.columns:
            # 按唯一ID分組，保留年份最大的記錄
            merged_speakers_df = merged_speakers_df.sort_values('year', ascending=False)
            merged_speakers_df = merged_speakers_df.drop_duplicates(subset=['unique_id'], keep='first')
...
def extract_session_features(sessions_df):
    ...
    if 'start' in enhanced_df.columns:
        ...
        if not pd.api.types.is_datetime64_dtype(enhanced_df['start']):
            enhanced_df['start'] = pd.to_datetime(enhanced_df['start'], errors='coerce')
    ...
    if 'start' in enhanced_df.columns and 'end' in enhanced_df.columns:
        ...
        if not pd.api.types.is_datetime64_dtype(enhanced_df['end']):
            enhanced_df['end'] = pd.to_datetime(enhanced_df['end'], errors='coerce')
        
        # 計算時長（分鐘）
        enhanced_df['duration_minutes'] = (enhanced_df['end'] - enhanced_df['start']).dt.total_seconds() / 60
    ...
    if 'title' in enhanced_df.columns:
        enhanced_df['title_length'] = enhanced_df['title'].fillna('').str.len()
    
    # 提取描述長度特徵
    if 'description' in enhanced_df.columns:
        enhanced_df['description_length'] = enhanced_df['description'].fillna('').str.len()
    
    # 提取講者數量特徵
    if 'speakers' in enhanced_df.columns:
        # 處理不同格式的speakers欄位
        def count_speakers(speakers):
            if isinstance(speakers, list):
                return len(speakers)
            elif isinstance(speakers, str):
                try:
                    return len(json.loads(speakers))
                except:
                    return len([s.strip() for s in speakers.split(',') if s.strip()])
            else:
                return 0
        
        enhanced_df['speaker_count'] = enhanced_df['speakers'].apply(count_speakers)
...
def main():
    ...
    for filename in tqdm(json_files, desc="處理JSON檔案"):
        ...
        # 保存單獨年份的資料
        if not sessions_df.empty:
            sessions_df.to_csv(f"{args.output_dir}/sessions_{year}.csv", index=False, encoding='utf-8')
            logger.info(f"已保存 {year} 年議程資料: {len(sessions_df)} 筆記錄")
        
        if not speakers_df.empty:
            speakers_df.to_csv(f"{args.output_dir}/speakers_{year}.csv", index=False, encoding='utf-8')
            logger.info(f"已保存 {year} 年講者資料: {len(speakers_df)} 筆記錄")
        
        if not tracks_df.empty:
            tracks_df.to_csv(f"{args.output_dir}/tracks_{year}.csv", index=False, encoding='utf-8')
            logger.info(f"已保存 {year} 年議程軌資料: {len(tracks_df)} 筆記錄")
    
    # 合併所有年份的資料
    merged_sessions_df, merged_speakers_df, merged_tracks_df = merge_yearly_data(yearly_data)
    ...
    if not enhanced_sessions_df.empty:
        enhanced_sessions_df.to_csv(f"{args.output_dir}/sessions.csv", index=False, encoding='utf-8')
        logger.info(f"已保存合併後的議程資料: {len(enhanced_sessions_df)} 筆記錄")
    
    if not merged_speakers_df.empty:
        merged_speakers_df.to_csv(f"{args.output_dir}/speakers.csv", index=False, encoding='utf-8')
        logger.info(f"已保存合併後的講者資料: {len(merged_speakers_df)} 筆記錄")
    
    if not merged_tracks_df.empty:
        merged_tracks_df.to_csv(f"{args.output_dir}/tracks.csv", index=False, encoding='utf-8')
        logger.info(f"已保存合併後的議程軌資料: {len(merged_tracks_df)} 筆記錄")
    ...
    if not yearly_data:
        ...
        stats_df.to_csv(f"{args.output_dir}/yearly_stats.csv", encoding='utf-8')
        logger.info(f"已保存年度統計資料")
