    ...
    for year in tqdm(years, desc="下載年度資料"):
        ...
        if os.path.exists(save_path):
            ...
            if file_size > 100:  # 檔案大小 > 100 bytes，假設內容完整
                logger.info(
                    f"檔案已存在，跳過下載: {save_path} ({file_size} bytes)"
                )
                results['skipped'] += 1
                continue
        # 下載檔案
...
def save_file_info(valid_files):
    ...
    # 保存為CSV
    pd.DataFrame(file_info).to_csv(
        'data/raw/file_info.csv', index=False, encoding='utf-8'
    )
    logger.info("已將檔案資訊保存到 data/raw/file_info.csv")
