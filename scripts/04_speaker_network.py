...
def build_speaker_network(sessions_df, speakers_df):
    ...
    for _, row in sessions_df.iterrows():
        ...
        if len(speakers_data) > 1:
            for i in range(len(speakers_data)):
                for j in range(i + 1, len(speakers_data)):
                    speaker1 = speakers_data[i]
                    speaker2 = speakers_data[j]
    ...
    for comm in set(partition.values()):
        comm_speakers = [
            (node, G.nodes[node]['name'],
             G.nodes[node].get('sessions_count', 0))
            for node, node_comm in partition.items() if node_comm == comm
        ]
        main_speakers[comm] = sorted(
            comm_speakers, key=lambda x: x[2], reverse=True
        )[:5]
...
def plot_speaker_network(G, partition):
    ...
    # 確保每個節點都有社群分配且是數值
    colors = [cmap(partition.get(node, -1)) for node in G.nodes() if node in partition and isinstance(partition[node], (int, float))]
    # 如果有節點沒有有效社群，需要處理，這裡暫時跳過這些節點的顏色設置
    # 根據演講次數調整節點大小
    node_sizes = [
        50 + G.nodes[node].get('sessions_count', 1) * 20 for node in G.nodes()
    ]
    # 根據合作次數調整邊的粗細
    ...
    nx.draw_networkx_nodes(
        G, pos, node_size=node_sizes, node_color=colors, alpha=0.8
    )
    nx.draw_networkx_edges(
        G, pos, width=edge_widths, alpha=0.3, edge_color='gray'
    )
    # 只為主要節點添加標籤
    big_nodes = [
        node for node in G.nodes()
        if G.nodes[node].get('sessions_count', 0) > 2
    ]
    labels = {node: G.nodes[node]['name'] for node in big_nodes}
    nx.draw_networkx_labels(
        G, pos, labels=labels, font_size=10, font_family='SimHei'
    )
...
def analyze_key_speakers(G):
    ...
    speakers_importance = pd.DataFrame({
        ...
        'sessions_count': [
            G.nodes[node].get('sessions_count', 0) for node in G.nodes()
        ],
    ...
    # 排序
    speakers_importance = speakers_importance.sort_values(
        by='importance_score', ascending=False
    )
    return speakers_importance


def analyze_speaker_participation_over_time(sessions_df):
    """分析講者參與度隨時間的變化"""
    # 建立年份到講者的映射
    ...
    speaker_trends = pd.DataFrame({
        ...
        'continuing_speakers': [
            len(continuing_speakers[year]) for year in years
        ]
    })
    # 繪製趨勢圖
    plt.figure(figsize=(12, 6))
    sns.lineplot(
        data=speaker_trends, x='year', y='total_speakers',
        marker='o', label='總講者數'
    )
    sns.lineplot(
        data=speaker_trends, x='year', y='new_speakers',
        marker='s', label='新講者'
    )
    sns.lineplot(
        data=speaker_trends, x='year', y='continuing_speakers',
        marker='^', label='持續參與講者'
    )
...
def main():
    ...
    # 建立講者網絡
    G, _ = build_speaker_network(sessions_df, speakers_df)
    # 社群檢測
    ...
    # 分析講者參與度隨時間的變化
    speaker_trends = analyze_speaker_participation_over_time(sessions_df)
    # 儲存結果
    ...
    with open('community_analysis.txt', 'w', encoding='utf-8') as f:
        ...
        for comm, count in sorted(
            ...
            if comm in main_speakers:
                f.write('主要講者:\n')
                for _, name, session_count in main_speakers[comm]:
                    f.write(f'  - {name}: {session_count} 場議程\n')
            f.write('\n')
